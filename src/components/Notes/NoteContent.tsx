...
export const NoteContent: React.FC<NoteContentInterface> = ({ menuOpen, setNoteUpdate}) => {
  ...
  const handleRemoveSmile = async () => {
    try {
      const data = await Axios.patch('/notes/update/' + selectNote._id, { smile: "" });
  ...
  useEffect(() => {
    try {
        const noteContent = async () => {
          if  (selectNote.name) {
            const data = await Axios.patch('/notes/update/' + selectNote._id, { smile: selectEmoji ? selectEmoji : selectNote.smile, imageUrl: image ? image : selectNote.imageUrl });
            // setNoteUpdate(true);
            setShowEmoji(false);
            dispatch(fetchNotes(data.data));
          }
  ...
  const handleRemoveImg = async () => {
    try {
      const data = await Axios.patch('/notes/update/' + selectNote._id, { imageUrl: "" });
      dispatch(fetchNotes(data.data));
      // setNoteUpdate(true);
    }
    catch (err) {
  ...
  const onChange = async (content: string) => {
    try {
      ...
      await Axios.patch('/notes/update/' + selectNote._id, { blocks: data });
    }
    catch (err) {
  ...
  useEffect(() => {
    const rename = async () => {
      if (isRename) {
        try {
          const data = await Axios.patch('/notes/update/' + selectNote._id, { name: isName });
          dispatch(fetchNotes(data.data));
        }
        catch (err) {
              ...
              <div className={`text-7xl flex items-center gap-3 ${selectNote.smile === "" ? 'hidden' : ''}`}>
                ...
                <div className='note-hover'>
                  <div className='note-setting py-4 flex gap-2 opacity-1 manage-note_btn relative'>
                    <button className={`flex cursor-pointer text-sm items-center gap-2 border border-light-grey hover:bg-btn-hover hover:opacity-100 opacity-90 rounded-md p-2 px-3 h-[36px] ${selectNote.smile ? 'hidden' : ''}`} onClick={() => setShowEmoji(true)}>                   
                    ...
                    <button className={`flex cursor-pointer text-sm gap-2 border border-light-grey hover:bg-btn-hover hover:opacity-100 opacity-90 rounded-md p-2 px-3 h-[36px] ${selectNote.imageUrl ? 'hidden' : ''}`} onClick={() => {handleOpenFile()}}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 22 22" fill="none">
                        <path d="M19.5081 3.21109H5.50815C4.40515 3.21109 3.50815 4.10809 3.50815 5.21109V19.2111C3.50815 20.3141 4.40515 21.2111 5.50815 21.2111H19.5081C20.6111 21.2111 21.5081 20.3141 21.5081 19.2111V5.21109C21.5081 4.10809 20.6111 3.21109 19.5081 3.21109ZM5.50815 19.2111V5.21109H19.5081L19.5101 19.2111H5.50815Z" fill="#333332"/>
                        <path d="M10.5081 14.2111L9.50815 13.2111L6.50815 17.2111H18.5081L13.5081 10.2111L10.5081 14.2111Z" fill="#333332"/>
                      </svg>
                  ...
                  {!isRename ? <h1 className='text-7xl font-bold text-noteName leading-none' onClick={handleRename}>{selectNote.name}</h1> : <textarea ref={textareaRef} value={isName} onChange={(e: any) => setName(e.target.value)} onKeyDown={onKeyDown} className='text-7xl font-bold text-noteName resize-none break-words outline-none' style={{height: '3rem'}}>{selectNote.name}</textarea>}
                </div>
                <div>
                    <div className='mt-2'>
                      <Editor key={JSON.stringify(selectNote.blocks)} onChange={onChange} initialContent={selectNote.blocks} />
                    </div>
                </div>
               </div>
            </div>
