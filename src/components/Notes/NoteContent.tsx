...
export const NoteContent: React.FC<NoteContentInterface> = ({ imageUrl, name, smile, _id, blocks, menuOpen, setNoteUpdate, isName, setName, setIsControl }) => {
  ...
  useEffect(() => {
    try {
        const noteContent = async () => {
          if  (name) {
            const data = await Axios.patch('/notes/update/' + _id, { smile: selectEmoji ? selectEmoji : smile, imageUrl: image ? image : imageUrl });
            setNoteUpdate(true);
            setShowEmoji(false);
            dispatch(fetchNotes(data.data));
          }
  ...
  useEffect(() => {
    const rename = async () => {
      if (isRename) {
        try {
          const data = await Axios.patch('/notes/update/' + _id, { name: isName });
          dispatch(fetchNotes(data.data));
        }
        catch (err) {
