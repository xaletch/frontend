...
export const Item: React.FC<DocType> = ({ _id, name, smile }) => {
    ...
    return (
        <div className='px-1'>
            <Link to={`/documents/${_id}`}>
                <div className={`page p-1 px-3 h-8 flex items-center justify-between font-medium cursor-pointer hover:bg-light-grey relative`}>
                    <div className='flex items-center'>
                        <div className='w-4 h-4 mr-1 rounded hover:bg-grey' onClick={(e) => {e.preventDefault(); handleOpenUnderNote(_id)}}>
                            <svg className={`h-4 w-4 ${isOpen }`} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ stroke: "#676767" }}>
                    ...
                    <div className='setting flex items-center gap-2'>
                        ...
                        <button className='w-4 h-4 flex justify-center items-center rounded hover:bg-grey' onClick={(e) => {e.preventDefault(); handleCreateNote()}}>
