...
export const Item: React.FC<DocType> = ({ _id, name, smile }) => {
  ...
  return (
    <div className="px-1">
      <Link to={`/documents/${_id}`}>
        ...
        >
          <div className="flex items-center">
            <div
              className="w-4 h-4 mr-1 rounded hover:bg-secondary-300"
              onClick={(e) => {
                e.preventDefault();
            ...
            <div className="flex">
              <span
                className="whitespace-nowrap text-base font-medium text-secondary-400"
                style={{ color: "#676767" }}
              >
          ...
          <div className="setting flex items-center gap-2">
            {/* onClick={actionNote} */}
            <button
              className="w-4 h-4 flex justify-center items-center rounded hover:bg-secondary-300"
      ...
      {isOpen && (
        <p
          className="text-[12px] font-medium text-secondary-400"
