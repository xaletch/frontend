...
export const Cart: React.FC = () => {
  ...
  return (
    <div className="fixed w-full h-screen top-0">
      <div className="w-[400px] h-[300px] shadow-xl bg-secondary-50 rounded-xl relative top-2/4 left-2/4 transform -translate-x-1/2 -translate-y-1/2 overflow-y-scroll scroll-smooth">
        <div className="p-5 h-full">
          <input
            className="outline-none w-full px-4 py-2 rounded-lg bg-white-50 shadow-sm text-secondary-900 text-base font-normal placeholder:text-base placeholder:font-normal placeholder:text-secondary-300"
            type="text"
            placeholder="Поиск по корзине"
          />
          <div className="flex flex-col gap-2 pt-3 pb-5">
            {cartItems?.map((items: CartItemInterface) => (
              <>
                {items?.notes.map((item, index) => (
                  <div
                    className="px-3 py-2 flex items-center justify-between rounded-lg bg-white-50 relative shadow-sm"
                    key={index}
                  >
                    ...
                    <div className="flex justify-end items-center absolute top-0 left-0 right-0 bottom-0 rounded-lg bg-secondary-50 z-30 shadow-sm opacity-0">
                      <div className="mr-3 flex items-center gap-2">
                        <button className="w-7 h-7 flex items-center justify-center rounded-md hover:bg-white-50 hover:shadow-sm duration-200 ease-linear">
                          <svg
                            width="14"
                            height="18"
                            viewBox="0 0 14 18"
