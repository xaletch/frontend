...
interface ManageNoteInterface {
    menuOpen: boolean;
    setAddNote: Dispatch<SetStateAction<boolean>>;
...
export const ManageNote: React.FC<ManageNoteInterface> = ({ menuOpen, setAddNote, name, title, imageUrl, smile, text, _id }) => {
    ...
    useEffect(() => {
        try {
            const noteContent = async () => {
                if (name) {
                    await Axios.patch('/notes/update/' + _id, { smile: selectEmoji ? selectEmoji : smile, imageUrl: image ? image : imageUrl });
                }
            }
            noteContent();
        }
                    ...
                    <div className='md:max-w-3xl lg:max-w-4xl mx-auto'>
                        <div className='pl-14 manage-note'>
                            ...
                            <div className='py-4 flex gap-2 opacity-1 manage-note_btn relative'>
                                <button className={`flex cursor-pointer text-sm items-center gap-2 border border-light-grey hover:bg-secondary rounded p-1 px-2 ${smile ? 'hidden' : ''}`} onClick={() => setShowEmoji(!showEmoji)}>                   
