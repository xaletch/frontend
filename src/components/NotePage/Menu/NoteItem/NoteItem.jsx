...
export const NoteItem = ({ name, id, noteName, smile, setNote, setNoteName, setIsUpdate, isUpdate, noteId, handleUpdate, handleSelectNote }) => {
          ...
          {isUpdate || noteId !== id ? 
            <Link to={`/${id}`}>
              <div className='flex'>
                ...
                <span className='whitespace-nowrap text-base' style={{color: '#676767'}}>{name.length >= 14 ? `${name.slice(0, 9)}...` : name}</span>
              </div>
            </Link>
          :
