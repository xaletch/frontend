...
import { Control } from '../../../Notes/Control/Control';
  ...
  return (
    ...
    <div className={`page p-1 px-3 h-8 flex items-center justify-between font-medium cursor-pointer hover:bg-light-grey relative`} onDoubleClick={handleUpdate} onClick={() => handleSelectNote(id)}>
      ...
      <div className='setting flex items-center gap-2'>
        ...
        {/* <Control name={name} username={'kirill'} controlCords={controlCords} setNote={setNote}/> */}
        {/* {!action && <Action action={action} setAction={setAction} handleDeleteNote={() => handleDeleteNote(id)} handleUpdate={handleUpdate} />} */}
        {/* <Action action={action} setAction={setAction} handleDeleteNote={() => handleDeleteNote(id)} handleUpdate={handleUpdate} /> */}
