...
export const Header: React.FC<MenuInterface> = ({ menuOpen, setMenuOpen, selectedNote }) => {
  ...
  return (
    <div className='absolute top-0 z-10' style={{width: `${menuOpen === false ? `calc(100%)` : `calc(100% - 240px)`}`, left: `${menuOpen === false ? `0` : `240px`}`}}>
      <nav className='p-2 px-3 flex h-14 items-center gap-4'>
        <svg className={`cursor-pointer ${menuOpen === true ? 'hidden' : ''}`} onClick={handleMenu} xmlns="http://www.w3.org/2000/svg" width="21" height="24" viewBox="0 0 16 12" fill="none">
          <path d="M0 0H16V2H0V0ZM0 5H16V7H0V5ZM0 10H16V12H0V10Z" fill="#CCCCCC"/>
        </svg>
        <div className='w-full flex justify-between'>
          <div className='text-sm h-auto p-1'>{}</div>
